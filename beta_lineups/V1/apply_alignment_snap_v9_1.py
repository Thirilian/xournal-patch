...
def main():
    ...
    # Le texte exact varie si le patch 8.6.8 a deja ete applique avant celui-ci: il enveloppe alors
    # cet appel dans un if/else (detection du mode de la ligne), sans le prefixe de type sur cette
    # ligne precise. Les deux formes sont gerees ici.
    text = cpp.read_text(encoding="utf-8")
    old_y_other_plain = 'std::vector<AlignmentCandidate> candidatesOther = buildCandidates(snapped.y, snapped.height, otherCenterFraction);'
    new_y_other_plain = ('std::vector<AlignmentCandidate> candidatesOther = buildCandidates(\n'
    ...
    old_y_other_wrapped = '            candidatesOther = buildCandidates(snapped.y, snapped.height, otherCenterFraction);'
    new_y_other_wrapped = ('            candidatesOther = buildCandidates(\n'
                          '                    snapped.y, snapped.height, otherCenterFraction,\n'
                          '                    isSmallMark(snapped.width, snapped.height) || isCrossShape(dynamic_cast<const Stroke*>(el)));')
    n_plain = text.count(old_y_other_plain)
    n_wrapped = text.count(old_y_other_wrapped)
    if n_plain == 2:
        text = text.replace(old_y_other_plain, new_y_other_plain)
        cpp.write_text(text, encoding="utf-8")
        print(f"[OK]    EditSelection.cpp: findAlignmentY - other ({n_plain} occurrences, forme simple)")
    elif n_wrapped == 2:
        text = text.replace(old_y_other_wrapped, new_y_other_wrapped)
        cpp.write_text(text, encoding="utf-8")
        print(f"[OK]    EditSelection.cpp: findAlignmentY - other ({n_wrapped} occurrences, forme enveloppee par 8.6.8)")
    elif text.count(new_y_other_plain) > 0 or text.count(new_y_other_wrapped) > 0:
        print("[SKIP]  EditSelection.cpp: findAlignmentY - other deja a jour.")
    else:
        print(f"[ECHEC] EditSelection.cpp: findAlignmentY - other - motif trouve {n_plain} (simple) / {n_wrapped} (enveloppe) fois (attendu 2 dans un des deux cas)")
        ok = False
    ...
    # Meme situation que pour Y ci-dessus : gere la forme simple et la forme enveloppee par 8.6.8.
    text = cpp.read_text(encoding="utf-8")
    old_x_other_plain = 'std::vector<AlignmentCandidate> candidatesOther = buildCandidates(snapped.x, snapped.width);'
    new_x_other_plain = ('std::vector<AlignmentCandidate> candidatesOther = buildCandidates(\n'
                         '                snapped.x, snapped.width, 0.5,\n'
                         '                isSmallMark(snapped.width, snapped.height) || isCrossShape(dynamic_cast<const Stroke*>(el)));')
    old_x_other_wrapped = '            candidatesOther = buildCandidates(snapped.x, snapped.width);'
    new_x_other_wrapped = ('            candidatesOther = buildCandidates(\n'
                          '                    snapped.x, snapped.width, 0.5,\n'
                          '                    isSmallMark(snapped.width, snapped.height) || isCrossShape(dynamic_cast<const Stroke*>(el)));')
    n_plain = text.count(old_x_other_plain)
    n_wrapped = text.count(old_x_other_wrapped)
    if n_plain == 2:
        text = text.replace(old_x_other_plain, new_x_other_plain)
        cpp.write_text(text, encoding="utf-8")
        print(f"[OK]    EditSelection.cpp: findAlignmentX - other ({n_plain} occurrences, forme simple)")
    elif n_wrapped == 2:
        text = text.replace(old_x_other_wrapped, new_x_other_wrapped)
        cpp.write_text(text, encoding="utf-8")
        print(f"[OK]    EditSelection.cpp: findAlignmentX - other ({n_wrapped} occurrences, forme enveloppee par 8.6.8)")
    elif text.count(new_x_other_plain) > 0 or text.count(new_x_other_wrapped) > 0:
        print("[SKIP]  EditSelection.cpp: findAlignmentX - other deja a jour.")
    else:
        print(f"[ECHEC] EditSelection.cpp: findAlignmentX - other - motif trouve {n_plain} (simple) / {n_wrapped} (enveloppe) fois (attendu 2 dans un des deux cas)")
        ok = False
