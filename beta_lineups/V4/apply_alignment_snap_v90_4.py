#!/usr/bin/env python3
"""
apply_alignment_snap_v90_4.py : version consolidee, regroupant les
patchs suivants (dans l'ordre) :
  v90, 11.1, 11.3, 11.5, 11.5.2, 11.5.3, 11.6

EXCLUS explicitement de cette consolidation, a la demande de
l'utilisateur : 11.2, 11.2.2, 11.4, 11.4.2 (retrait a la hausse du
retrecissement des fleches equidistantes ; nouvelle condition de
passage Top/Middle/Below - ces deux sujets restent a retravailler plus
tard).

Contenu fonctionnel : le systeme complet d'accroche/alignement
...
l'onglet Preferences "Snapping" avec ses 9 cases a cocher et 10
variables numeriques configurables, le template de traduction, PLUS :
  - 11.1 : guides toujours en trait plein, meme avec un style de
    trait pointille/points selectionne
  - 11.3 : le verrou de position sur une grande ligne n'est applique
    que si la famille de petites lignes forme reellement une grille
    reguliere valide
  - 11.5 : couleurs des guidelines (gris eclairci pour le centrage de
    page, preservation de l'accroche ordinaire sur l'axe perpendicu-
    laire d'une ligne boostee, guide rouge quand graduation assist ne
    peut pas verrouiller)
  - 11.5.2 : correctif d'un parametre largeur/hauteur inverse dans la
    verification de validite de grille du patch 11.3
  - 11.5.3 : une ligne de famille de graduation reste rose meme en
    mode Middle (au lieu de virer au vert)
  - 11.6 : le point d'ancrage vertical d'une textbox utilise
    desormais la fraction configurable meme quand elle est
    self (et non plus uniquement quand elle est l'autre element), et
    une textbox multi-lignes utilise toujours le vrai centre
    geometrique

Modifie 24 fichiers + cree 2 nouveaux fichiers
(src/core/undo/LineRepositionUndoAction.h/.cpp).

NECESSITE : apply_arrow_resize_fix_v2.py (prerequis separe, a
appliquer avant ce script).

A lancer depuis la racine du depot xournalpp, sur un depot vierge (ou
tout du moins sans qu'aucun patch individuel de cette liste n'ait deja
ete applique).
"""
import sys
...
F01_OLD0 = """\"you have the shape recognizer turned on while writing.\"
msgstr \"\"
...
msgstr \"\"
"""
F01_NEW0 = """\"you have the shape recognizer turned on while writing.\"
msgstr \"\"
...
msgstr \"\"
"""
F01_OLD1 = """#: ../resources-templates/com.github.xournalpp.xournalpp.xml.in:12
msgid \"Xournal++ page template file\"
msgstr \"\""""
F01_NEW1 = """#: ../resources-templates/com.github.xournalpp.xournalpp.xml.in:12
msgid \"Xournal++ page template file\"
msgstr \"\"
...
msgid \"Settings\"
msgstr \"\""""
F02_OLD0 = """    this->actionDB->setActionState(Action::GRID_SNAPPING, enable);
}

auto Control::getTextEditor() -> TextEditor* {
    if (this->win) {
        return this->win->getXournal()->getTextEditor();"""
F02_NEW0 = """    this->actionDB->setActionState(Action::GRID_SNAPPING, enable);
}
...
F05_NEW0 = """    this->snapGrid = true;
    ...
    this->strokeRecognizerMinSize = 40;
"""
F05_OLD1 = """        this->snapRotationTolerance = tempg_ascii_strtod(reinterpret_cast<const char*>(value), nullptr);
...
F05_NEW1 = """        this->snapRotationTolerance = tempg_ascii_strtod(reinterpret_cast<const char*>(value), nullptr);
    } else if (xmlStrcmp(name, reinterpret_cast<const xmlChar*>(\"snapGrid\")) == 0) {
        this->snapGrid = xmlStrcmp(value, reinterpret_cast<const xmlChar*>(\"true\")) == 0;
...
F06_NEW1 = """    bool snapGrid{};
     ...
     */
    std::u8string defaultSaveName;  // should be string - don't change to path"""
F07_OLD0 = """auto ArrowHandler::createShape(bool isAltDown, bool isShiftDown, bool isControlDown)
...
"""
F07_NEW0 = """auto ArrowHandler::createShape(bool isAltDown, bool isShiftDown, bool isControlDown)
        -> std::pair<std::vector<Point>, Range> {
    Point c = snappingHandler.snap(this->currPoint, this->startPoint, isAltDown);
    ...
    const ArrowKind kind = this->doubleEnded ? ArrowKind::DOUBLE : ArrowKind::SINGLE;
"""
F08_OLD0 = """#include \"BaseShapeHandler.h\"

#include <cmath>   // for pow, NAN
#include <memory>  // for make_unique, __share...
"""
F08_NEW0 = """#include \"BaseShapeHandler.h\"

#include <algorithm>  // for min, max
#include <cmath>   // for pow, NAN
#include <memory>  // for make_unique, __share...
"""
F08_OLD1 = """#include \"view/overlays/ShapeToolView.h\"           // for ShapeToolView
...
BaseShapeHandler::BaseShapeHandler(Control* control, const PageRef& page, bool flipShift, bool flipControl):
        ...
        flipShift(flipShift),"""
F08_NEW1 = """#include \"view/overlays/ShapeToolView.h\"           // for ShapeToolView
...
BaseShapeHandler::BaseShapeHandler(Control* control, const PageRef& page, bool flipShift, bool flipControl):
        InputHandler(control, page),
        flipShift(flipShift),"""
F08_OLD2 = """void BaseShapeHandler::updateShape(bool isAltDown, bool isShiftDown, bool isControlDown) {
...
F08_NEW2 = """void BaseShapeHandler::updateShape(bool isAltDown, bool isShiftDown, bool isControlDown) {
    auto [shape, rg] = this->createShape(isAltDown, isShiftDown, isControlDown);
    std::swap(shape, this->shape);
...
auto BaseShapeHandler::createView(xoj::view::Repaintable* parent) const -> std::unique_ptr<xoj::view::OverlayView> {"""
F09_OLD0 = """#pragma once

#include <memory>  // for shared_ptr
#include <utility>  // for pair
#include <vector>   // for vector
"""
F09_NEW0 = """#pragma once
...
#include <vector>   // for vector
"""
F09_OLD1 = """    const std::vector<Point>& getShape() const;
...
F09_NEW1 = """    const std::vector<Point>& getShape() const;
...
protected:
    std::vector<Point> shape;
"""
F09_OLD3 = """    Point buttonDownPoint;  // used for tapSelect and filtering - never snapped to grid.
    Point startPoint;       // May be snapped to grid

    std::shared_ptr<xoj::util::DispatchPool<xoj::view::ShapeToolView>> viewPool;
};"""
F09_NEW3 = """    Point buttonDownPoint;  // used for tapSelect and filtering - never snapped to grid.
    Point startPoint;       // May be snapped to grid
...
F10_OLD0 = """#include \"gui/XournalppCursor.h\"                   // for XournalppCursor
...
F10_NEW0 = """#include \"gui/XournalppCursor.h\"                   // for XournalppCursor
#include \"model/Document.h\"                        // for Document
#include \"model/Element.h\"                         // for Element::Index
#include \"model/ElementInsertionPosition.h\"
#include \"model/Layer.h\"                          // for Layer
...
#include \"util/i18n.h\"                            // for _
#include \"util/serializing/ObjectInputStream.h\"   // for ObjectInputStream"""
F10_OLD1 = """constexpr int DELETE_PADDING = 20;
...
static auto computeBoxes(const InsertionOrder& elts) -> std::pair<Range, Range> {
    return std::transform_reduce(
            ...
            [](auto&& p, auto&& q) {"""
F10_NEW1 = """constexpr int DELETE_PADDING = 20;
constexpr int ROTATE_PADDING = 8;
...
static auto computeBoxes(const InsertionOrder& elts) -> std::pair<Range, Range> {
    return std::transform_reduce(
            elts.begin(), elts.end(), std::pair<Range, Range>(),
            [](auto&& p, auto&& q) {"""
F10_OLD2 = """EditSelection::EditSelection(Control* ctrl, InsertionOrder elts, const PageRef& page, Layer* layer, XojPageView* view,
...
F10_NEW2 = """EditSelection::EditSelection(Control* ctrl, InsertionOrder elts, const PageRef& page, Layer* layer, XojPageView* view,
                             const Range& bounds, const Range& snappingBounds):
        snappedBounds(snappingBounds),
        btnWidth(getBtnWidth(ctrl)),
        sourcePage(page),
...
void EditSelection::paint(cairo_t* cr, double zoom) {
    ...
    if (std::abs(this->rotation) > std::numeric_limits<double>::epsilon()) {"""
F10_OLD9 = """        cairo_translate(cr, -rx, -ry);
    }
...
F10_NEW9 = """        cairo_translate(cr, -rx, -ry);
    }
    this->contents->paint(cr, x, y, this->rotation, this->width, this->height, zoom);
...
#include <utility>  // for pair
#include <vector>   // for vector"""
F11_OLD1 = """class DeleteUndoAction;
class LineStyle;
class ObjectInputStream;
class ObjectOutputStream;
class XojFont;
class Document;"""
F11_NEW1 = """class DeleteUndoAction;
class LineStyle;
class ObjectInputStream;
class Settings;
class ObjectOutputStream;
class XojFont;
class Document;"""
F11_OLD2 = """    Layer* sourceLayer{};

    /**
     * The contents of the selection
     */
    std::unique_ptr<EditSelectionContents> contents;"""
F11_NEW2 = """    Layer* sourceLayer{};
     ...
     */
    std::unique_ptr<EditSelectionContents> contents;"""
F12_OLD0 = """        width = (this->modControl) ? std::hypot(width, height) :
                                     std::copysign(std::max(std::abs(width), std::abs(height)), width);
        height = std::copysign(width, height);
    }

    double radiusX = 0;"""
F12_NEW0 = """        width = (this->modControl) ? std::hypot(width, height) :
                                     std::copysign(std::max(std::abs(width), std::abs(height)), width);
        height = std::copysign(width, height);
    } else {
    ...
    double radiusX = 0;"""
F13_OLD0 = """auto RulerHandler::createShape(bool isAltDown, bool isShiftDown, bool isControlDown)
        -> std::pair<std::vector<Point>, Range> {
...
F13_NEW0 = """auto RulerHandler::createShape(bool isAltDown, bool isShiftDown, bool isControlDown)
        -> std::pair<std::vector<Point>, Range> {
    Point secondPoint = snappingHandler.snap(this->currPoint, this->startPoint, isAltDown);
...
#include \"model/SplineSegment.h\"                 // for SplineSegment
#include \"model/Stroke.h\"                        // for Stroke"""
F14_OLD1 = """#include \"undo/UndoRedoHandler.h\"                // for UndoRedoHandler
#include \"util/Assert.h\"                         // for xoj_assert
#include \"util/DispatchPool.h\"
#include \"view/overlays/SplineToolView.h\"

SplineHandler::SplineHandler(Control* control, const PageRef& page):"""
F14_NEW1 = """#include \"undo/UndoRedoHandler.h\"                // for UndoRedoHandler
#include \"util/Assert.h\"                         // for xoj_assert
#include \"util/DispatchPool.h\"
...
F14_NEW4 = """
    ...
    if (this->isButtonPressed) {
        ...
        if (this->inFirstKnotAttractionZone) {
            // The button was pressed within the attraction zone. Wait for unpress to confirm/deny spline finalization
            return true;"""
F14_OLD5 = """        bool nowInAttractionZone =
...
F14_NEW5 = """        bool nowInAttractionZone =
                this->buttonDownPoint.lineLengthTo(this->knots.front()) < this->knotsAttractionRadius;
        if (nowInAttractionZone) {
            this->activeGuideX.reset();
            this->activeGuideY.reset();
 ...
 */
struct SplineHandlerData {"""
F15_OLD1 = """    const Point& currPoint;
    double knotsAttractionRadius;
    bool closedSpline;
};

/**"""
F15_NEW1 = """    const Point& currPoint;
    double knotsAttractionRadius;
    bool closedSpline;
...
/**"""
F15_OLD2 = """    bool inFirstKnotAttractionZone = false;
...
F15_NEW2 = """    bool inFirstKnotAttractionZone = false;
    SnapToGridInputHandler snappingHandler;
    ...
    static constexpr double KNOTS_ATTRACTION_RADIUS_IN_PIXELS = 10.0;  // for circling the spline's knots"""
F16_OLD0 = """    MOVE_SELECTION_LAYER_DOWN,
    ROTATION_SNAPPING,
    GRID_SNAPPING,
    PREFERENCES,

    // Menu View"""
F16_NEW0 = """    MOVE_SELECTION_LAYER_DOWN,
    ROTATION_SNAPPING,
    GRID_SNAPPING,
    ...
    // Menu View"""
F17_OLD0 = """        \"move-selection-layer-down\",
        \"rotation-snapping\",
        \"grid-snapping\",
        \"preferences\",
        \"paired-pages-mode\",
        \"paired-pages-offset\","""
F17_NEW0 = """        \"move-selection-layer-down\",
        \"rotation-snapping\",
        \"grid-snapping\",
        ...
        \"paired-pages-mode\",
        \"paired-pages-offset\","""
F18_OLD0 = """            builder.get(\"cbAutosave\"), \"toggled\",
...
F18_NEW0 = """            builder.get(\"cbAutosave\"), \"toggled\",
            G_CALLBACK(+[](SettingsDialog* self) { self->enableWithCheckbox(\"cbAutosave\", \"boxAutosave\"); }), this);
...
F18_OLD1 = """    return gtk_range_get_value(range);
}

/**
 * Checkbox was toggled, enable / disable it
 */"""
F18_NEW1 = """    return gtk_range_get_value(range);
}
 ...
 * Checkbox was toggled, enable / disable it
 */"""
F18_OLD2 = """    loadCheckbox(\"cbShowScrollbarLeft\", settings->isScrollbarOnLeft());
...
F18_NEW2 = """    loadCheckbox(\"cbShowScrollbarLeft\", settings->isScrollbarOnLeft());
    loadCheckbox(\"cbAutoloadMostRecent\", settings->isAutoloadMostRecent());
    loadCheckbox(\"cbAutoloadXoj\", settings->isAutoloadPdfXoj());
    ...
    loadCheckbox(\"cbAddVerticalSpace\", settings->getAddVerticalSpace());
    loadCheckbox(\"cbAddHorizontalSpace\", settings->getAddHorizontalSpace());"""
F18_OLD3 = """    disableWithCheckbox(\"cbUnlimitedScrolling\", \"cbAddHorizontalSpace\");
...
F18_NEW3 = """    disableWithCheckbox(\"cbUnlimitedScrolling\", \"cbAddHorizontalSpace\");
    ...
    enableWithEnabledCheckbox(\"cbAddVerticalSpace\", \"spAddVerticalSpaceAbove\");
    enableWithEnabledCheckbox(\"cbAddHorizontalSpace\", \"spAddHorizontalSpaceRight\");"""
F18_OLD4 = """    settings->setScrollbarOnLeft(getCheckbox(\"cbShowScrollbarLeft\"));
...
F18_NEW4 = """    settings->setScrollbarOnLeft(getCheckbox(\"cbShowScrollbarLeft\"));
    settings->setAutoloadMostRecent(getCheckbox(\"cbAutoloadMostRecent\"));
    settings->setAutoloadPdfXoj(getCheckbox(\"cbAutoloadXoj\"));
    ...
    settings->setAddVerticalSpace(getCheckbox(\"cbAddVerticalSpace\"));
    settings->setAddHorizontalSpace(getCheckbox(\"cbAddHorizontalSpace\"));"""
F19_OLD0 = """#pragma once

#include <functional>
#include <string>  // for string
#include <vector>  // for vector
"""
F19_NEW0 = """#pragma once
...
#include <vector>  // for vector
"""
F19_OLD1 = """    void loadSlider(const char* name, double value);
...
F19_NEW1 = """    void loadSlider(const char* name, double value);
    double getSlider(const char* name);
...
"""
F23_OLD0 = """     <attribute name=\"label\" translatable=\"yes\">Grid Snapping</attribute>
     <attribute name=\"action\">win.grid-snapping</attribute>
    </item>
   </section>
   <section>
    <item>"""
F23_NEW0 = """     <attribute name=\"label\" translatable=\"yes\">Grid Snapping</attribute>
     <attribute name=\"action\">win.grid-snapping</attribute>
    </item>
    <item>
...
F24_NEW0 = """              <packing>
            ...
            <child>
              <object class=\"GtkBox\" id=\"toolsTabBox\">
                ...
                <property name=\"margin-start\">5</property>
                <property name=\"orientation\">vertical</property>"""
F24_OLD1 = """                    <property name=\"fill\">True</property>
                    <property name=\"position\">0</property>
                  </packing>
                </child>
              </object>
...
F24_NEW1 = """                    <property name=\"fill\">True</property>
                    <property name=\"position\">0</property>
                  </packing>
                </child>
              </object>
            ...
            <child>
              <object class=\"GtkBox\" id=\"audioRecordingTabBox\">
                <property name=\"name\">audioRecordingTabBox</property>"""
F24_OLD2 = """                    <property name=\"fill\">True</property>
                    <property name=\"position\">0</property>
                  </packing>
                </child>
              </object>
            ...
            <child>
              <object class=\"GtkBox\" id=\"latexTabBox\">
                <property name=\"visible\">True</property>"""
F24_NEW2 = """                    <property name=\"fill\">True</property>
                    <property name=\"position\">0</property>
                  </packing>
                </child>
              </object>
            ...
            <child>
              <object class=\"GtkBox\" id=\"latexTabBox\">
                <property name=\"visible\">True</property>"""
F24_OLD3 = """                    <property name=\"fill\">True</property>
                    <property name=\"position\">0</property>
                  </packing>
                </child>
              </object>
            ...
            <child>
              <object class=\"GtkBox\" id=\"paletteTabBox\">
                <property name=\"visible\">True</property>"""
F24_NEW3 = """                    <property name=\"fill\">True</property>
                    <property name=\"position\">0</property>
                  </packing>
                </child>
              </object>
            ...
            <child>
              <object class=\"GtkBox\" id=\"paletteTabBox\">
                <property name=\"visible\">True</property>"""
F24_OLD4 = """                <property name=\"orientation\">vertical</property>
                <child>
                  <placeholder/>
                </child>
              </object>
          ...
          <packing>
            <property name=\"expand\">True</property>"""
F24_NEW4 = """                <property name=\"orientation\">vertical</property>
                <child>
                  <placeholder/>
                </child>
              </object>
...
def main():
    paths = {
        "f01": Path("po/xournalpp.pot"),
        "f02": Path("src/core/control/Control.cpp"),
        "f03": Path("src/core/control/Control.h"),
        "f04": Path("src/core/control/actions/ActionProperties.h"),
        "f05": Path("src/core/control/settings/Settings.cpp"),
        "f06": Path("src/core/control/settings/Settings.h"),
        "f07": Path("src/core/control/tools/ArrowHandler.cpp"),
        "f08": Path("src/core/control/tools/BaseShapeHandler.cpp"),
        "f09": Path("src/core/control/tools/BaseShapeHandler.h"),
        "f10": Path("src/core/control/tools/EditSelection.cpp"),
        "f11": Path("src/core/control/tools/EditSelection.h"),
        "f12": Path("src/core/control/tools/EllipseHandler.cpp"),
        "f13": Path("src/core/control/tools/RulerHandler.cpp"),
        "f14": Path("src/core/control/tools/SplineHandler.cpp"),
        "f15": Path("src/core/control/tools/SplineHandler.h"),
        "f16": Path("src/core/enums/Action.enum.h"),
        "f17": Path("src/core/enums/generated/Action.NameMap.generated.h"),
        "f18": Path("src/core/gui/dialog/SettingsDialog.cpp"),
        "f19": Path("src/core/gui/dialog/SettingsDialog.h"),
        "f20": Path("src/core/model/Stroke.cpp"),
        "f21": Path("src/core/view/overlays/ShapeToolView.cpp"),
        "f22": Path("src/core/view/overlays/SplineToolView.cpp"),
        "f23": Path("ui/mainmenubar.xml"),
        "f24": Path("ui/settings.glade"),
    }
    ...
    if "ALIGNMENT_GROUP_TOLERANCE_PX" in paths["f10"].read_text(encoding="utf-8"):
        print("[SKIP] Ce patch consolide (v90_4) semble deja applique.")
        sys.exit(0)
    ...
    ok &= apply_edit(paths["f01"], F01_OLD0, F01_NEW0, "po/xournalpp.pot: zone 1/2")
    ok &= apply_edit(paths["f01"], F01_OLD1, F01_NEW1, "po/xournalpp.pot: zone 2/2")
    ok &= apply_edit(paths["f02"], F02_OLD0, F02_NEW0, "src/core/control/Control.cpp: zone 1/1")
    ok &= apply_edit(paths["f03"], F03_OLD0, F03_NEW0, "src/core/control/Control.h: zone 1/1")
    ok &= apply_edit(paths["f04"], F04_OLD0, F04_NEW0, "src/core/control/actions/ActionProperties.h: zone 1/1")
    ok &= apply_edit(paths["f05"], F05_OLD0, F05_NEW0, "src/core/control/settings/Settings.cpp: zone 1/4")
    ok &= apply_edit(paths["f05"], F05_OLD1, F05_NEW1, "src/core/control/settings/Settings.cpp: zone 2/4")
    ok &= apply_edit(paths["f05"], F05_OLD2, F05_NEW2, "src/core/control/settings/Settings.cpp: zone 3/4")
    ok &= apply_edit(paths["f05"], F05_OLD3, F05_NEW3, "src/core/control/settings/Settings.cpp: zone 4/4")
    ok &= apply_edit(paths["f06"], F06_OLD0, F06_NEW0, "src/core/control/settings/Settings.h: zone 1/2")
    ok &= apply_edit(paths["f06"], F06_OLD1, F06_NEW1, "src/core/control/settings/Settings.h: zone 2/2")
    ok &= apply_edit(paths["f07"], F07_OLD0, F07_NEW0, "src/core/control/tools/ArrowHandler.cpp: zone 1/1")
    ok &= apply_edit(paths["f08"], F08_OLD0, F08_NEW0, "src/core/control/tools/BaseShapeHandler.cpp: zone 1/7")
    ok &= apply_edit(paths["f08"], F08_OLD1, F08_NEW1, "src/core/control/tools/BaseShapeHandler.cpp: zone 2/7")
    ok &= apply_edit(paths["f08"], F08_OLD2, F08_NEW2, "src/core/control/tools/BaseShapeHandler.cpp: zone 3/7")
    ok &= apply_edit(paths["f08"], F08_OLD3, F08_NEW3, "src/core/control/tools/BaseShapeHandler.cpp: zone 4/7")
    ok &= apply_edit(paths["f08"], F08_OLD4, F08_NEW4, "src/core/control/tools/BaseShapeHandler.cpp: zone 5/7")
    ok &= apply_edit(paths["f08"], F08_OLD5, F08_NEW5, "src/core/control/tools/BaseShapeHandler.cpp: zone 6/7")
    ok &= apply_edit(paths["f08"], F08_OLD6, F08_NEW6, "src/core/control/tools/BaseShapeHandler.cpp: zone 7/7")
    ok &= apply_edit(paths["f09"], F09_OLD0, F09_NEW0, "src/core/control/tools/BaseShapeHandler.h: zone 1/4")
    ok &= apply_edit(paths["f09"], F09_OLD1, F09_NEW1, "src/core/control/tools/BaseShapeHandler.h: zone 2/4")
    ok &= apply_edit(paths["f09"], F09_OLD2, F09_NEW2, "src/core/control/tools/BaseShapeHandler.h: zone 3/4")
    ok &= apply_edit(paths["f09"], F09_OLD3, F09_NEW3, "src/core/control/tools/BaseShapeHandler.h: zone 4/4")
    ok &= apply_edit(paths["f10"], F10_OLD0, F10_NEW0, "src/core/control/tools/EditSelection.cpp: zone 1/10")
    ok &= apply_edit(paths["f10"], F10_OLD1, F10_NEW1, "src/core/control/tools/EditSelection.cpp: zone 2/10")
    ok &= apply_edit(paths["f10"], F10_OLD2, F10_NEW2, "src/core/control/tools/EditSelection.cpp: zone 3/10")
    ok &= apply_edit(paths["f10"], F10_OLD3, F10_NEW3, "src/core/control/tools/EditSelection.cpp: zone 4/10")
    ok &= apply_edit(paths["f10"], F10_OLD4, F10_NEW4, "src/core/control/tools/EditSelection.cpp: zone 5/10")
    ok &= apply_edit(paths["f10"], F10_OLD5, F10_NEW5, "src/core/control/tools/EditSelection.cpp: zone 6/10")
    ok &= apply_edit(paths["f10"], F10_OLD6, F10_NEW6, "src/core/control/tools/EditSelection.cpp: zone 7/10")
    ok &= apply_edit(paths["f10"], F10_OLD7, F10_NEW7, "src/core/control/tools/EditSelection.cpp: zone 8/10")
    ok &= apply_edit(paths["f10"], F10_OLD8, F10_NEW8, "src/core/control/tools/EditSelection.cpp: zone 9/10")
    ok &= apply_edit(paths["f10"], F10_OLD9, F10_NEW9, "src/core/control/tools/EditSelection.cpp: zone 10/10")
    ok &= apply_edit(paths["f11"], F11_OLD0, F11_NEW0, "src/core/control/tools/EditSelection.h: zone 1/3")
    ok &= apply_edit(paths["f11"], F11_OLD1, F11_NEW1, "src/core/control/tools/EditSelection.h: zone 2/3")
    ok &= apply_edit(paths["f11"], F11_OLD2, F11_NEW2, "src/core/control/tools/EditSelection.h: zone 3/3")
    ok &= apply_edit(paths["f12"], F12_OLD0, F12_NEW0, "src/core/control/tools/EllipseHandler.cpp: zone 1/1")
    ok &= apply_edit(paths["f13"], F13_OLD0, F13_NEW0, "src/core/control/tools/RulerHandler.cpp: zone 1/1")
    ok &= apply_edit(paths["f14"], F14_OLD0, F14_NEW0, "src/core/control/tools/SplineHandler.cpp: zone 1/7")
    ok &= apply_edit(paths["f14"], F14_OLD1, F14_NEW1, "src/core/control/tools/SplineHandler.cpp: zone 2/7")
    ok &= apply_edit(paths["f14"], F14_OLD2, F14_NEW2, "src/core/control/tools/SplineHandler.cpp: zone 3/7")
    ok &= apply_edit(paths["f14"], F14_OLD3, F14_NEW3, "src/core/control/tools/SplineHandler.cpp: zone 4/7")
    ok &= apply_edit(paths["f14"], F14_OLD4, F14_NEW4, "src/core/control/tools/SplineHandler.cpp: zone 5/7")
    ok &= apply_edit(paths["f14"], F14_OLD5, F14_NEW5, "src/core/control/tools/SplineHandler.cpp: zone 6/7")
    ok &= apply_edit(paths["f14"], F14_OLD6, F14_NEW6, "src/core/control/tools/SplineHandler.cpp: zone 7/7")
    ok &= apply_edit(paths["f15"], F15_OLD0, F15_NEW0, "src/core/control/tools/SplineHandler.h: zone 1/3")
    ok &= apply_edit(paths["f15"], F15_OLD1, F15_NEW1, "src/core/control/tools/SplineHandler.h: zone 2/3")
    ok &= apply_edit(paths["f15"], F15_OLD2, F15_NEW2, "src/core/control/tools/SplineHandler.h: zone 3/3")
    ok &= apply_edit(paths["f16"], F16_OLD0, F16_NEW0, "src/core/enums/Action.enum.h: zone 1/1")
    ok &= apply_edit(paths["f17"], F17_OLD0, F17_NEW0, "src/core/enums/generated/Action.NameMap.generated.h: zone 1/1")
    ok &= apply_edit(paths["f18"], F18_OLD0, F18_NEW0, "src/core/gui/dialog/SettingsDialog.cpp: zone 1/5")
    ok &= apply_edit(paths["f18"], F18_OLD1, F18_NEW1, "src/core/gui/dialog/SettingsDialog.cpp: zone 2/5")
    ok &= apply_edit(paths["f18"], F18_OLD2, F18_NEW2, "src/core/gui/dialog/SettingsDialog.cpp: zone 3/5")
    ok &= apply_edit(paths["f18"], F18_OLD3, F18_NEW3, "src/core/gui/dialog/SettingsDialog.cpp: zone 4/5")
    ok &= apply_edit(paths["f18"], F18_OLD4, F18_NEW4, "src/core/gui/dialog/SettingsDialog.cpp: zone 5/5")
    ok &= apply_edit(paths["f19"], F19_OLD0, F19_NEW0, "src/core/gui/dialog/SettingsDialog.h: zone 1/2")
    ok &= apply_edit(paths["f19"], F19_OLD1, F19_NEW1, "src/core/gui/dialog/SettingsDialog.h: zone 2/2")
    ok &= apply_edit(paths["f20"], F20_OLD0, F20_NEW0, "src/core/model/Stroke.cpp: zone 1/2")
    ok &= apply_edit(paths["f20"], F20_OLD1, F20_NEW1, "src/core/model/Stroke.cpp: zone 2/2")
    ok &= apply_edit(paths["f21"], F21_OLD0, F21_NEW0, "src/core/view/overlays/ShapeToolView.cpp: zone 1/2")
    ok &= apply_edit(paths["f21"], F21_OLD1, F21_NEW1, "src/core/view/overlays/ShapeToolView.cpp: zone 2/2")
    ok &= apply_edit(paths["f22"], F22_OLD0, F22_NEW0, "src/core/view/overlays/SplineToolView.cpp: zone 1/1")
    ok &= apply_edit(paths["f23"], F23_OLD0, F23_NEW0, "ui/mainmenubar.xml: zone 1/1")
    ok &= apply_edit(paths["f24"], F24_OLD0, F24_NEW0, "ui/settings.glade: zone 1/5")
    ok &= apply_edit(paths["f24"], F24_OLD1, F24_NEW1, "ui/settings.glade: zone 2/5")
    ok &= apply_edit(paths["f24"], F24_OLD2, F24_NEW2, "ui/settings.glade: zone 3/5")
    ok &= apply_edit(paths["f24"], F24_OLD3, F24_NEW3, "ui/settings.glade: zone 4/5")
    ok &= apply_edit(paths["f24"], F24_OLD4, F24_NEW4, "ui/settings.glade: zone 5/5")
