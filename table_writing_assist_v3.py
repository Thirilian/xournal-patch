#!/usr/bin/env python3
"""
table_writing_assist_v3.py : version consolidee, fusionnant
l'integralite de la serie "aide a l'ecriture d'un tableau" en un seul
script - remplace table_writing_assist.py (v2, patchs 12.1 a 12.6) en
y ajoutant les patchs 12.7 a 12.11.
...
(retrecissement si debordement, jusqu'a 6pt minimum ; ragrandissement
vers la taille explicitement choisie par l'utilisateur des que la
place le permet a nouveau) et recentrage dynamique pendant la frappe.
Les fleches directionnelles en bout de texte permettent de naviguer
...
Contenu (patchs fusionnes, dans l'ordre) :
  ...
  12.4    : cote manquant base sur l'extremite de la ligne adjacente +
            marge de 1 unite pour la verification de debordement
  12.5    : une textbox existante parfaitement centree retrouve le
            mode table a sa reouverture
  12.6    : la cible du rescale a la hausse est la taille explicitement
            choisie par l'utilisateur (Settings::getFont())
  12.7    : correctif du bug de fusion fantome entre deux tableaux
            distincts + subdivision en emplacements ouverts (colonne
            bornee gauche/droite/haut, ouverte en bas)
  12.8    : detection restreinte aux traits visibles a l'ecran +
            intersection geometrique stricte entre cotes adjacents
  12.9    : un cote qui ne se connecte a rien est ecarte (pas toute la
            case rejetee)
  12.10   : la subdivision en emplacements ne se declenche que s'il y
            a la place pour au moins deux emplacements
  12.11   : regle non reversible pour les tableaux a colonnes
            multiples avec une seule ligne d'en-tete horizontale pres
            du haut (hauteur de ligne geometrique, repetee sans
            espace)
...
static auto detectTableCell(double x, double y, Layer* layer, double fontSize,
    ...
    if (hasLeft && hasRight && hasTop) {
        ...
        if (crossedVerticalsCount >= 3 && rowHeight > 0 && rowHeight <= 0.25 * verticalsLength) {
            double rowTop;
            double rowBottom;
            if (y < topY) {
...
def main():
    ...
    if "tableMode" in h_file.read_text(encoding="utf-8"):
        print("[SKIP] Ce patch (table_writing_assist_v3) semble deja applique.")
        sys.exit(0)
